...
const Signup: React.FC<SignupProps> = (props) => {
  ...
  const t = useTranslations('RewardPoint.Signup')
  ...
  return <div className="min-h-[14.33rem] h-auto mx-4 bg-white rounded-2xl px-[0.83rem] py-[1.33rem] pb-8" style={{ marginTop: signType === 'Default' ? '-3.16rem' : '0' }}>
    <h4 className="text-2xl font-Inter-SemiBold leading-6 mb-[1.33rem] text-primary font-semibold" >{t('Title')}</h4>

    <PointProgress points={0} />

    <p className="text-[1rem] text-primary font-medium font-Inter-Medium mt-4 mb-[1.67rem]">
      {t('PointInitial')}: <span className="text-boldColor">0</span>{signType === "Invitee" && `/${TOTAL_REWARD_POINTS}`}
    </p>

    {
      signType === 'Invitee' &&
      <p className="text-base font-Inter-Regular font-normal leading-[1.67rem]">{t('SignupDesc')}</p>
    }
    ...
    <div className="flex justify-center mt-[1.67rem]">
      ...
      >
        {signType === 'Default' ? t('SignupBtn_Default') : t('SignupBtn_Share')}
      </button>
    </div>
