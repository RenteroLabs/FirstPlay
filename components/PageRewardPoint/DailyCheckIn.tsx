...
const DailyCheckIn: React.FC<DailyCheckInPorps> = (props) => {
  ...
  return <div className={`bg-white rounded-2xl mt-[0.83rem] px-[0.83rem] pt-[0.92rem] ${type !== 'home' && 'mx-4'} ${type === 'home' ? 'pb-3' : 'pb-6'} ${type !== 'home' && 'mb-[0.83]'}`}>
    <div className="flex justify-between items-center mb-[1.33rem]">
      <h4 className="text-2xl font-Inter-SemiBold leading-6 text-primary font-semibold mb-0" >{t('DailyCheckInSectionTitle')}</h4>
      ...
      >
        {isCheckIned ? t('CheckedInBtn') : t('CheckInBtn')}
    ...
    <p className="text-[1rem] font-Inter-Medium font-medium leading-[1.33rem] mt-[1.33rem] mb-0">
      {t('DailyCheckInTip', { checkedNum: checkedNum, unCheckedNum: 7 - checkedNum })}
    </p>
  </div>
}
