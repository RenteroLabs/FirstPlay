...
import { getInviteePointDetail } from "services/invitepoint";
import { useAccount } from "wagmi";
import { isEmpty } from "lodash";
import { formatAddress } from "util/format";
import InviteeModal from "./InviteeListModal";
import { useTranslations } from "next-intl";

interface InviteFriendProps {
  ownCode: string | undefined
}
const InviteFriend: React.FC<InviteFriendProps> = (props) => {
  const { ownCode } = props
  const t = useTranslations('RewardPoint')

  const { address } = useAccount()
  const [inviteeList, setInviteeList] = useState<Record<string, any>[]>([])
  ...
  const [twitterLink, telegramLink] = useMemo(() => {
    const shareText = `I am participating in the Earn Points and Easily Win $2 activity. Join me and invite friends to win cash! `

    const twitterShare = `https://twitter.com/share?text=${shareText}&url=${shareLink}`
    const telegramShare = `https://telegram.me/share/url?url=${shareLink}&text=${shareText}`

    return [twitterShare, telegramShare]
  }, [shareLink])


  const { run: queryInviteeList } = useRequest(getInviteePointDetail, {
    manual: true,
    onSuccess: ({ data }) => {
      console.log(data)
      setInviteeList(data || [])
  ...
  return <div className="bg-white rounded-2xl mx-4 my-[0.83rem] px-3 py-[1.33rem]">
    <h4 className="text-2xl text-primary font-semibold font-Inter-SemiBold leading-6 mb-[0.67rem]">{t('InviteSectionTitle')}</h4>
    <p className="text-base text-primary font-Inter-Regular font-normal leading-[1.17rem] mb-[2.17rem]">{t('InviteSectionSubtitle')}</p>
    ...
    <div className="relative h-[19.5rem] bg-[rgba(142,80,228,0.08)] rounded-2xl px-[0.83rem] ">
      ...
      <div className="flex justify-around items-center mb-[1.67rem]  pt-[4.25rem]">
        <div className="flex flex-col justify-center items-center ">
          ...
          <p className="text-[1rem] text-boldColor font-Inter-Medium font-medium mb-0">{t('ShareInvite_First')}</p>
        </div>
        ...
        <div className="flex flex-col justify-center items-center ">
          ...
          <p className="text-[1rem] text-boldColor font-Inter-Medium font-medium mb-0">{t('ShareInvite_Second')}</p>
        </div>
      </div>
      <Divider className="text-base font-Inter-SemiBold font-semibold leading-[1.17rem]" >{t('InviteWord')}</Divider>
      <div className="flex justify-around mt-[1.67rem]">
        <a target="_blank" href={twitterLink} rel="noreferrer">
          <div className="flex flex-col items-center min-w-[4.8rem]">
        ...
        <CopyToClipboard text={shareLink}>
          <div className="flex flex-col items-center min-w-[4.8rem]" onClick={handleCopyLink}>
            <div className="h-[3.67rem] w-[3.67rem] rounded-full bg-primary inline-flex items-center justify-center">
            ...
            <span className="underline mt-[0.83rem] text-[1rem] font-Inter-Medium font-medium leading-4">{t('CopyLink')}</span>
          </div>
        </CopyToClipboard>
      </div>
    </div>
    ...
    <h5 className="flex justify-between items-center mb-[1.33rem] mt-[1.67rem]">
      <p className="text-primary text-base font-semibold font-Inter-SemiBold leading-[1.17rem] mb-0">{t('InviteesTitle')}</p>
      <span className="text-primary/50 text-[1rem] font-Inter-Medium font-medium leading-4 cursor-pointer underline" onClick={() => setShowInviteeModal(true)}>{t('ViewAllInviteeList')}</span>
    </h5>
    {
      isEmpty(inviteeList) ?
        <div className="text-primary/50">{t('NoneInviteeYet')}</div>
        :
        <div className="flex items-center justify-between text-base text-primary font-Inter-Regular font-normal leading-[1.17rem]">
          ...
          <p className="glow-1 mb-0 mx-[0.67rem]" style={{ flexGrow: 1 }}>{formatAddress(inviteeList[0]?.invitee, 4)} {inviteeList[0]?.type}</p>
          <span className="flex-none">+{inviteeList[0]?.change}</span>
        </div>
    }
    <InviteeModal
      showModal={showInviteeModal}
      setShowModal={setShowInviteeModal}
